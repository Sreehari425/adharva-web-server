...
fn update_event(event_name: &str, status: &str, state: &rocket::State<SharedEvents>) -> Json<Vec<EventDetail>> {
    ...
    if let Ok(mut events) = events_lock.try_lock() {
    // 👇 Save current state first
    save_current_state(&events)?;

    // ...perform updates...
    let updated_json = serde_json::to_string_pretty(&events)?;
    fs::write("events.json", updated_json)?;
}
